...
**有効期間は「今」ではなく「タイムスタンプの時点」で判定します。** ペイロードがトークンを持つとき、`-attime` が検査をそのトークンの主張する瞬間に固定し、レポートはそれがいつだったかを述べます。署名用証明書は、誰かが検証するずっと前に有効期間を終えます。それが**今日**有効かを訊くのは、誰も持っていない問いに答えることです。トークンが無ければ基準にできるものが無く、検査は時計のままです。

トークンの `genTime` は証明ではありません —— この段はタイムスタンプをトークン自身の最後の証明書に対して照合するので、自己整合的な偽造は任意の時刻を名乗れます。それでもこちらを採るのは、代替が `--allow-expired`（時刻を一切問わない）だからです。
